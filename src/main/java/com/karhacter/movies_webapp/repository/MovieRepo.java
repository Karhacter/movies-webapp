...
public interface MovieRepo extends JpaRepository<Movie, Long> {
    ...
    Page<Movie> findByCategory(@Param("category") Category category, Pageable pageable);
    ...
    // Searching Movies
    @Query(value = "SELECT * FROM movies m WHERE LOWER(m.title) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(m.description) LIKE LOWER(CONCAT('%', :search, '%'))", nativeQuery = true)
