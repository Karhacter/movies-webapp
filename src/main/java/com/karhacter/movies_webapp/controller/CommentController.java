...
public class CommentController {
    ...
    @GetMapping("/movie/slug/{slug}/paged")
    public ResponseEntity<List<CommentDTO>> getCommentsByMovieSlugPaged(
            @PathVariable String slug,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size) {
        List<CommentDTO> comments = commentService.getCommentsByMovieSlugPaged(slug, page, size);
        return new ResponseEntity<>(comments, HttpStatus.OK);
    }
