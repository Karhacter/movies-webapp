...
public class ReviewServiceImpl implements ReviewService {
    ...
    @Override
    public List<ReviewDTO> searchReviews(String content, String username, String movieTitle, String reviewDate, Integer rating) {
        List<Review> reviews = reviewRepo.searchReviews(content, rating);
    ...
    @Override
    @Transactional
    public ReviewDTO createOrUpdateRating(ReviewDTO reviewDTO) {
        User user = userRepo.findById(reviewDTO.getUserID())
                .orElseThrow(() -> new ResourceNotFoundException("User", "id", reviewDTO.getUserID()));
        Movie movie = movieRepo.findById(reviewDTO.getMovieID())
                .orElseThrow(() -> new ResourceNotFoundException("Movie", "id", reviewDTO.getMovieID()));

        Review review = reviewRepo.findByUserAndMovie(user, movie).orElse(null);

        if (review == null) {
            review = new Review();
            review.setUser(user);
            review.setMovie(movie);
            review.setComment(""); // empty comment for rating-only
        }
